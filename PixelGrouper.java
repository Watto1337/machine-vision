...
public class PixelGrouper {
    public static ArrayList<PixelGroup> findPixelGroups(Pixel[] pixels, double maxLength) {
        ArrayList<PixelGroup> groups = new ArrayList<>();

        for (Pixel pixel : pixels) {
            if (pixel != null) {
                ArrayList<PixelGroup> validGroups = null;
    ...
    public static boolean groupsMatch(PixelGroup a, PixelGroup b, double maxRatioDif, double maxMovement) {
        ...
        double distance = Math.sqrt(Math.pow(aCenterX - bCenterX, 2) + Math.pow(aCenterY - bCenterY, 2));
